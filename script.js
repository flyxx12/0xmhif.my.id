...
const items = document.querySelectorAll('.s-track'); // Pakai class spotify baru
let currentTrack = 0;

// Fungsi klik tombol Love (Berguna, bukan pajangan)
const heartBtn = document.querySelector('.heart');
if(heartBtn) {
    heartBtn.addEventListener('click', function() {
        this.querySelector('i').classList.toggle('fas');
        this.querySelector('i').classList.toggle('far');
        this.style.color = this.querySelector('i').classList.contains('fas') ? '#1DB954' : '#b3b3b3';
    });
}

function playSelected(index) {
    // Visual update di list
    items.forEach(item => item.classList.remove('active'));
    items[index].classList.add('active');
    
    currentTrack = index;
    const source = items[index].getAttribute('data-src');
    audio.src = source;
    
    // GANTI TEKS "Pilih Lagu" jadi Judul Lagu (Bukan pajangan)
    const trackName = items[index].querySelector('.s-name').innerText;
    trackTitle.innerText = trackName;
    
    audio.load();
    audio.play().then(() => {
        playBtn.innerHTML = '<i class="fas fa-pause"></i>';
    }).catch(err => console.log("Izin audio diperlukan"));
}

function togglePlay() {
    // Logic anti-macet: kalau src kosong, tembak lagu 1
    if (!audio.src || audio.src === "" || window.location.href.includes(audio.getAttribute('src'))) {
        playSelected(0);
    } else {
        if (audio.paused) {
            audio.play();
...
function nextTrack() {
    currentTrack = (currentTrack + 1) % items.length;
    playSelected(currentTrack);
}

function prevTrack() {
    currentTrack = (currentTrack - 1 + items.length) % items.length;
    playSelected(currentTrack);
}
